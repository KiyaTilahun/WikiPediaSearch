<template>
      ...
      <div class="relative">
        ...
        <ul v-if="filteredSuggestions.length"
          ...
          <li v-for="(item, index) in filteredSuggestions" :key="index" @click="selectSuggestion(item)"
            class="px-4 py-2 text-gray-900 cursor-pointer hover:bg-gray-200 dark:text-white dark:hover:bg-gray-600">
            {{ item }}
          </li>
        </ul>
  ...
  <div v-if="searchresult.length" class="flex justify-center space-x-4 my-4">
    ...
    <button @click="nextPage" :disabled="!hasMoreResults" class="px-4 py-2 bg-gray-300 rounded disabled:opacity-50">
      Next
    </button>
  </div>
</template>
...
const onInput=async (e)=>{
  ...
  contents: [
    {
      parts: [
        { text: `Give me Some Words and Phrases that start with${e.target.value} as array 5 elements ,and just give me the elements to make it copyable` }
      ]
    }
  ]
};
await axios.post(apiUrl, requestData, {
  ...
  .then(response => {
    ...
    const match = responseText.match(/\[([^\]]+)\]/);
        ...
        if (match) {
          // Parse the matched string to a proper array
          const arrayString = match[0]; // This will give "[k,k,k]"
          const textArray = JSON.parse(arrayString); // Converts it to a JavaScript array

          // Log the array
          console.log('Extracted Array:', textArray);

          // Update the suggestions with the extracted array
          suggestions.value = textArray;
        }
  })
  .catch(error => {
...
const filteredSuggestions = computed(() =>
  suggestions.value.filter((item) =>
    item.toLowerCase().includes(searchName.value.toLowerCase())
  )
);
